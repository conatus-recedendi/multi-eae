...
def transfer(split, MAX_LEN=600):
    ...
    for d in tqdm(data):
        ...
        for entity_mention in entity_mentions:
            entity_information[entity_mention["id"]] = (
                entity_mention["start"],
                entity_mention["end"] - 1,
            )  # mention对应在整篇文章中的span位置编号
        ...
        for i, event_mention in enumerate(event_mentions):
            ...
            trigger_b, trigger_e, trigger_sent = (
                trigger["start"],
                trigger["end"] - 1,
                trigger["sent_idx"],
            )
            ...
            context = [
                [tup[0] for tup in sentences[trigger_sent][:-1][0]]
            ]  # 触发词所在句子的具体上下文内容
            if cur_len > MAX_LEN:  # 如果句子长度超过最大长度
                left_bound = sum([len(s[:-1][0]) for s in sentences[:trigger_sent]])
                right_bound = (
                    sum([len(s[:-1][0]) for s in sentences[: trigger_sent + 1]]) - 1
                )
                left_w = trigger_b - 1
                right_w = trigger_e + 1
                ...
                while (
                    cur_len < MAX_LEN
                    and left_sen >= 0
                    and right_sen < len(sentences)
                    and cur_len != prev_len
                ):
                    prev_len = cur_len
                    if left_sen >= 0:
                        left_sen_tokens = [
                            tup[0] for tup in sentences[left_sen][:-1][0]
                        ]
                    ...
                    if right_sen < len(sentences):
                        right_sen_tokens = [
                            tup[0] for tup in sentences[right_sen][:-1][0]
                        ]
            ...
            new_d["evt_triggers"] = [
                [trigger_b - offset, trigger_e - offset, [[event_type, 1.0]]]
            ]
            ...
            for argument in arguments:
                ...
                new_d["gold_evt_links"].append(
                    [
                        [trigger_b - offset, trigger_e - offset],
                        [span_b - offset, span_e - offset],
                        role,
                    ]
                )
                ...
                assert (
                    trigger_b - offset >= 0
                    and trigger_b - offset < len(all_context)
                    and trigger_e - offset >= 0
                    and trigger_e - offset < len(all_context)
                )
                ...
                if (
                    span_b - offset >= 0
                    and span_b - offset < len(all_context)
                    and span_e - offset >= 0
                    and span_e - offset < len(all_context)
                ):
                    assert (
                        tokens[span_b : span_e + 1]
                        == all_context[span_b - offset : span_e - offset + 1]
                    )
                else:
            ...
            for coref_entity in coref_entities:
                ...
                for entity in coref_entity:
                    ...
                    if (
                        entity_span_b >= 0
                        and entity_span_b < len(all_context)
                        and entity_span_e >= 0
                        and entity_span_e < len(all_context)
                    ):
            ...
            for entity_mention in entity_mentions:
                ...
                if (
                    entity_b >= 0
                    and entity_b < len(all_context)
                    and entity_e >= 0
                    and entity_e < len(all_context)
                ):
